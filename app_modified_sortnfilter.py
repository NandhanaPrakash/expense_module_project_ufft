...
@app.route('/sort_expenses', methods=["POST"])
def sort_expenses():
...
def filter_expenses():
    min_amount = request.form.get('min_amount')  # Minimum amount input from the frontend
    max_amount = request.form.get('max_amount')  # Maximum amount input from the frontend
    category = request.form.get('category')  # Category input from the frontend

    # Initialize base query and parameters
    query = "SELECT e.* FROM expenses e JOIN categories c ON e.category_id = c.category_id WHERE 1=1"
    params = []

    # Filter by amount range if both min and max values are provided
    if min_amount and max_amount:
        query += " AND e.amount BETWEEN %s AND %s"
        params.extend([float(min_amount), float(max_amount)])

    # Filter by category if a valid category is provided
    if category:
    ...
    # Return the filtered expenses data for rendering on the frontend
    return render_template('expenses.html', expenses=filtered_expenses)
